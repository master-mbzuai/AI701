...
if __name__ == "__main__":
        ...
        for folder in os.listdir(path):      
            ...
            for exp in os.listdir(path + folder):                        
                if("architecture.txt" == exp):                
                    ...
                    print(meta)          
                    results[path][folder]["mac_classifier"] = meta["CLASSIFIER_Params"]   
                    quantity = meta["CLASSIFIER_Params"]
                    quantity2 = meta["BACKBONE_Params"]                
    ...
    plt.title('Number of Learnable parameters vs accuracy of the models.')
    plt.xlabel('Learnable parameters')
    plt.ylabel('Accuracy')
    #plt.colorbar(label='Parameters (KMac)')
    plt.grid(True)
    plt.savefig("../results/_images/" + "-".join(legends) + "-params.jpg")
    plt.show()    
